...
parser.add_argument('--checkpoint_path', type=str, default='checkpoints/latest_model_data.ckpt',help='The path to the latest checkpoint weights for your model.')
parser.add_argument('--crop_height', type=int, default=512, help='Height of cropped input image to network')
parser.add_argument('--crop_width', type=int, default=512, help='Width of cropped input image to network')
# parser.add_argument('--model', type=str, default="UNet-Skip", help='The model you are using')
parser.add_argument('--dataset', type=str, default="data", help='The dataset you are using')
...
for ind in range(len(test_input_names)):
    ...
    #pred_image
    cv2.imwrite("%s/%s.png"%("test", file_name),cv2.cvtColor(np.uint8(out_vis_image), cv2.COLOR_RGB2BGR))
    #ground_truth_image
    cv2.imwrite("%s/%s.png"%("gt", file_name),cv2.cvtColor(np.uint8(gt), cv2.COLOR_RGB2BGR))
